...
uses
  ...
  U_UpdateFuncionario, Vcl.Imaging.pngimage, uDm_Query, U_Noti, Vcl.Mask;
...
type
  ...
  TFormCadastros = class(TForm)
    ...
    Label2: TLabel;
    Panel2: TPanel;
    pnl_HeaderEsp: TPanel;
    edt_PesqEspecialidade: TLabeledEdit;
    pnl_PesqEsp: TPanel;
    db_Epecialidade: TDBGrid;
    pnl_FooterEsp: TPanel;
    Label3: TLabel;
    pnl_DesativarEsp: TPanel;
    pnl_ReativarEsp: TPanel;
    Panel11: TPanel;
    ds_Especialidades: TDataSource;
    pnl_NovaEspecialidade: TPanel;
    edt_NomeEsp: TLabeledEdit;
    edt_Adicional: TMaskEdit;
    Label4: TLabel;
    Label5: TLabel;
    Label6: TLabel;
    Panel4: TPanel;
    Label7: TLabel;
    Panel5: TPanel;
    pnl_EditarEsp: TPanel;
    edt_NewEsp: TLabeledEdit;
    edt_NewAdd: TMaskEdit;
    Label8: TLabel;
    btn_SalvarEsp: TPanel;
    ...
    procedure db_EpecialidadeDrawColumnCell(Sender: TObject; const Rect: TRect;
      DataCol: Integer; Column: TColumn; State: TGridDrawState);
    procedure pnl_NovaEspecialidadeClick(Sender: TObject);
    procedure db_EpecialidadeCellClick(Column: TColumn);
    procedure btn_EditarEspClick(Sender: TObject);
  private
...
begin
  ...
  else if DBGrid1.DataSource.DataSet.FieldByName('cargo').AsString = '0' then
  begin
    DBGrid1.Canvas.Brush.Color := clInactiveCaption;
  end
  else if DBGrid1.DataSource.DataSet.FieldByName('cargo').AsString = '1' then
  begin
...
procedure TFormCadastros.db_EpecialidadeCellClick(Column: TColumn);
begin
  edt_NewEsp.Text := DmQuery.especialidades.FieldByName
    ('especialidade').AsString;
  edt_NewAdd.Text := DmQuery.especialidades.FieldByName('adicional').AsString;
end;

procedure TFormCadastros.db_EpecialidadeDrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  if db_Epecialidade.DataSource.DataSet.FieldByName('status').AsString = 'inativo'
  then
  begin
    db_Epecialidade.Canvas.Brush.Color := clred;
  end
  else if Odd(db_Epecialidade.DataSource.DataSet.RecNo) then
  begin
    db_Epecialidade.Canvas.Brush.Color := clsilver;
  end
  else
  begin
    db_Epecialidade.Canvas.Brush.Color := clwhite;
  end;
  if (gdSelected in State) then
  begin
    db_Epecialidade.Canvas.Brush.Color := $00BA2F00;
    db_Epecialidade.Canvas.Font.Color := clwhite;
  end;
  db_Epecialidade.Canvas.FillRect(Rect);
  db_Epecialidade.DefaultDrawColumnCell(Rect, DataCol, Column, State);
  db_Epecialidade.Canvas.TextRect(Rect, Rect.Left + 3, Rect.Top + 8,
    Column.Field.DisplayText);
end;
...
begin
  ...
  then
  begin
    db_Pacientes.Canvas.Brush.Color := clred;
  end
...
procedure TFormCadastros.btn_EditarEspClick(Sender: TObject);
begin
  pnl_EditarEsp.Visible := true;
end;
...
begin
  ...
  DmFuncionarios.cb_Especialidades.SQL.Clear;
  DmFuncionarios.cb_Especialidades.SQL.add
    ('Select * from especialidades WHERE status = ''ativo''');
  DmFuncionarios.cb_Especialidades.Open;
...
begin
  FormCadFuncinarios.tb_Endereco.TabVisible := true;
  FormCadFuncinarios.tb_InfBasicas.TabVisible := true;
  FormCadFuncinarios.tb_Func.Visible := false;
...
begin
  ...
  TDbGridPadrao(db_Pacientes).ClientHeight :=
    (30 * TDbGridPadrao(db_Pacientes).RowCount) + 30;
  TDbGridPadrao(db_Epecialidade).ScrollBars := ssVertical;
  TDbGridPadrao(db_Epecialidade).DefaultRowHeight := 30;
  TDbGridPadrao(db_Epecialidade).ClientHeight :=
    (30 * TDbGridPadrao(db_Epecialidade).RowCount) + 30;
end;
...
procedure TFormCadastros.pnl_NovaEspecialidadeClick(Sender: TObject);
var
  add: Currency;
begin
  add := StrToCurr(edt_Adicional.Text);
  DmQuery.Query.SQL.Clear;
  DmQuery.Query.SQL.add
    ('INSERT INTO especialidades (adicional, especialidade) VALUES (:adicional, :especialidade)');
  DmQuery.Query.ParamByName('adicional').AsCurrency := add;
  DmQuery.Query.ParamByName('especialidade').AsString := edt_NomeEsp.Text;
  DmQuery.Query.ExecSQL;
  DmQuery.especialidades.Refresh;
end;
