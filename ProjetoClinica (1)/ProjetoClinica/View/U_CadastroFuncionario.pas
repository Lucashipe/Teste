...
uses
  ...
  Vcl.CheckLst, Vcl.WinXPickers, U_Noti, Vcl.StdCtrls, Data.DB;
...
type
  ...
  TFormCadFuncinarios = class(TForm)
    ...
    edt_Crm: TEdit;
    lb_crm: TLabel;
    lb_Espcialidades: TLabel;
    cb_Especialidades: TComboBox;
    ds_Combobox: TDataSource;
...
begin
  ...
  begin
    ...
    lb_crm.Visible := true;
    lb_Espcialidades.Visible := true;
    cb_Especialidades.Visible := true;
  end
  else
  begin
    Medico := false;
    ...
    lb_Espcialidades.Visible := false;
    cb_Especialidades.Visible := false;
  end;
end;
...
begin
  ...
  DmQuery.Query.SQL.Add
    ('INSERT INTO funcionarios (pf_id, login, senha, cargo) VALUES (:pf_id, :login, :senha, :cargo) RETURNING id_funcionario');
  ...
  DmQuery.Query.Open;
  LastIdFunc := DmQuery.Query.FieldByName('id_funcionario').AsInteger;
  DmQuery.Query.SQL.Clear;

  if Medico = true then
  begin
    DmQuery.Query.SQL.Add
      ('INSERT INTO medicos (func_id, crm, especialidade) VALUES (:id_func, :crm, :especialidade)');
    DmQuery.Query.ParamByName('id_func').AsInteger := LastIdFunc;
    DmQuery.Query.ParamByName('crm').AsString := edt_Crm.Text;
...
begin
  ...
  while not DmFuncionarios.cb_Especialidades.Eof do
  begin
    cb_Especialidades.AddItem(DmFuncionarios.cb_Especialidades.FieldByName
      ('especialidade').AsString, nil);
    DmFuncionarios.cb_Especialidades.Next;
  end;
