...
begin
  ...
  pnl_ConfirmarAg.Visible := false;
  cb_Especialidades.Clear;
  cb_TempoEstimado.Clear;
  DmQuery.Query.SQL.Clear;
  edt_PacienteCpf.Clear;
  edt_Horario.Clear;

  FormNoti.Show;
  FormNoti.lb_noti.Caption := 'Consulta agendada com sucesso!';
  FormNoti.Timer1.Enabled := true;
  FormNoti.btn_Sim.Hide;
  FormNoti.btn_n.Hide;
end;

procedure TFormAgendamentos.btn_NaoClick(Sender: TObject);
begin
  pnl_ConfirmarAg.Visible := false;
  cb_Especialidades.Clear;
  cb_TempoEstimado.Clear;
...
begin
  ...
  DmQuery.Query.SQL.Clear;
  DmQuery.Query.SQL.add
    ('SELECT *, CAST(CONCAT(''Dr(a). '' ,nome, '' '', sobrenome) AS VARCHAR(100)) AS nome_completo_string FROM medicos');
